...
- 另附STM32代码，使用Keil5编写，工程文件在 ` Project ` 目录下，名为 ` 工程模板.uvprojx `。源代码在`APP`目录下。

# 注意事项
- 如果移植到如`Arduino Uno`、`Arduino Nano`、`89C51`、`STC产品`等工作电压为5V的单片机，务必转换串口电平，可参考[电平转换电路](https://mp.weixin.qq.com/s/EIcMxW0amMTJZTIobH1mqA)（未验证），移植到以上平台的时候注意删除`Blinker`的所有相关函数，并且注意中断服务函数的要求。
